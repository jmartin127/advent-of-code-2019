...
func main() {
	...
	// load the last state of the outputs
	finalOutputs := make([]*computerOutput, 0)
	for i := len(outputs) - 1; i > 0; i-- { // iterate in reverse order to get the final state
		output := outputs[i]
		existingOutput := outputForPosition(output.x, output.y, finalOutputs)
		if existingOutput == nil {
			finalOutputs = append(finalOutputs, output)
		}
	}

	// count the number of block tiles (tileID = 2)
	var numBlockTiles int
	for _, output := range finalOutputs {
		if output.tileID == 2 {
			numBlockTiles++
		}
	}
	fmt.Printf("Num block tiles: %d\n", numBlockTiles)
}
...
func readData() {
	...
	file, err := os.Open("/Users/jeff.martin@getweave.com/Documents/advent-of-code/day13/input.txt")
	if err != nil {
		panic(err)
	}
	defer file.Close()
...
func runComputer(outputChannel chan *computerOutput, original []int, wg *sync.WaitGroup) {
	...
	var input int // NOT used for this one
