...
// The rest of the panels are still black, but it looks like the robot was expecting to start on a white panel, not a black
// one.
...
func main() {
	...
	input <- 1 // First panel is white
	output := make(chan *computerOutput, 4)

	// initialize the robot and panels
	panels := make([]*panel, 0)
	startingPanel := &panel{x: 0, y: 0, color: 1}
	robot := &robot{
		facing: 0, // start's facing up.
		panel:  startingPanel,
	}
	panels = append(panels, startingPanel)
...
func printPanels(panels []*panel) {
	...
	for i := 0; i < height; i++ {
		matrix = append(matrix, make([]string, width))
		for j := 0; j < width; j++ {
			matrix[i][j] = " "
		}
	}
...
func readData() {
	...
	file, err := os.Open("/Users/jeff.martin@getweave.com/Documents/advent-of-code/day11/part2/input.txt")
	if err != nil {
		panic(err)
	}
	defer file.Close()
